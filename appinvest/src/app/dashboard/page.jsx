...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-black via-gray-900 to-emerald-900 animate-gradient" />
      <div className="absolute inset-0 pointer-events-none">
        {stars.map((star, i) => (
          ...
          >
            <FaStar className="w-3 h-3 text-emerald-300 opacity-60" />
          </div>
        ))}
      </div>
      <div className="relative z-10 container mx-auto px-4 py-6">
        <div className={`text-center mb-8 ${isLoaded ? 'animate-slide-in-up' : 'opacity-0'}`}>
          <div className="relative inline-block">
            ...
            <div className="absolute -top-2 -right-8">
              <FaBolt className="w-6 h-6 text-yellow-400 animate-bounce" />
            </div>
          </div>
          <p className="text-emerald-400 text-lg animate-pulse">Espace Membre</p>
        </div>
        <div className="grid grid-cols-3 gap-4 mb-8">
          {DASHBOARD_ITEMS.map((item, index) => (
            <div
              key={item.id}
              className={`bg-white/90 rounded-lg shadow-lg hover:shadow-xl border-4 border-red-600 p-4 text-center transform hover:scale-105 transition-all duration-300 ${
            ...
            >
              <Link href={item.href} onClick={item.id === 'logout' ? handleLogout : undefined}>
                <div className="space-y-3 group">
                  <div className={`w-14 h-14 bg-gradient-to-br ${item.color} rounded-full flex items-center justify-center mx-auto shadow-lg hover:rotate-6 transition-all duration-300`}>
                    <item.icon className="w-7 h-7 text-white" />
                  </div>
                  <p className="text-gray-900 text-xs font-medium leading-tight group-hover:text-red-600 transition-colors duration-300">
                    {item.label}
                  </p>
                </div>
              </Link>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50">
          <div className="bg-black/30 border-t border-emerald-500/20 backdrop-blur-xl">
            <div className="flex justify-around items-center py-3">
              {BOTTOM_TABS.map((tab, index) => (
                <Link key={tab.id} href={tab.href}>
                  <button
                    className={`flex flex-col items-center space-y-1 text-emerald-400 hover:text-white hover:bg-emerald-600/20 px-3 py-2 transition-all duration-300 ${
